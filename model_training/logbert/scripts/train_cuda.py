#!/usr/bin/env python3
"""
LogBERT 모델 NVIDIA CUDA 학습 스크립트
GPU 서버용 (NVIDIA GPU)
"""
...
# train_intel.py와 동일한 나머지 코드를 사용
# IntelLogBERTTrainer 클래스를 GPULogBERTTrainer로 변경하되,
# IPEX 최적화 부분만 제거

class GPULogBERTTrainer:
    """NVIDIA GPU 최적화 LogBERT 학습 클래스"""
    ...
    def train(self, train_dataloader, num_epochs: int):
        """전체 학습 과정"""
        logger.info("=" * 80)
        logger.info("🚀 NVIDIA GPU LogBERT 학습 시작")
        logger.info("=" * 80)
...
# train_intel.py의 load_config, get_data_files, main 함수를 그대로 사용
# 단, main()에서 IntelLogBERTTrainer 대신 GPULogBERTTrainer 사용
...
def main():
    ...
    parser = argparse.ArgumentParser(description='LogBERT NVIDIA CUDA 학습')
    parser.add_argument('--config', type=str, required=True,
                       help='설정 파일 경로 (예: configs/full_gpu.yaml)')
    ...
    # GPU 확인
    logger.info("=" * 80)
    logger.info("NVIDIA GPU LogBERT 학습 스크립트")
    logger.info("=" * 80)
    ...
    dataloader = create_dataloader(
        ...
        pin_memory=True,  # CUDA에서는 pin_memory 활성화
    )
    ...
    # 학습기 생성
    logger.info("\n" + "=" * 80)
    logger.info("GPU 학습기 초기화...")
    logger.info("=" * 80)
    
    trainer = GPULogBERTTrainer(config)
