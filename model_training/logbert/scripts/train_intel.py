#!/usr/bin/env python3
"""
LogBERT 모델 Intel GPU 학습 스크립트
Intel Arc Graphics (XPU) 지원
"""
...
try:
    ...
    print("⚠️  Intel Extension for PyTorch가 설치되어 있지 않습니다.")
    print("설치: pip install intel-extension-for-pytorch")
...
def setup_logging(log_file: Path = None):
    ...
    # Windows에서 콘솔 인코딩 먼저 설정 (핸들러 생성 전에)
    ...
    # 콘솔 핸들러 (이미 UTF-8로 설정된 stdout 사용)
...
def get_device():
    ...
    if IPEX_AVAILABLE and hasattr(torch, 'xpu') and torch.xpu.is_available():
        ...
        logger.info("⚠️  CPU 모드 (GPU를 사용할 수 없습니다)")
        return device, 'cpu'


class IntelLogBERTTrainer:
    """Intel GPU 최적화 LogBERT 학습 클래스"""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.device_type == 'xpu' and IPEX_AVAILABLE:
            ...
            self.model, self.optimizer = ipex.optimize(
                ...
                dtype=torch.float32  # 또는 torch.bfloat16
            )
    ...
    def train(self, train_dataloader, num_epochs: int):
        """전체 학습 과정"""
        logger.info("=" * 80)
        logger.info("🚀 Intel GPU LogBERT 학습 시작")
        logger.info("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='LogBERT Intel GPU 학습')
    ...
    parser.add_argument('--log-file', type=str, default=None,
                       help='로그 파일 경로 (기본: logs/train_YYYYMMDD_HHMMSS.log)')
    ...
    if args.log_file:
        ...
        # 기본 로그 파일: logs/train_YYYYMMDD_HHMMSS.log
        script_dir = Path(__file__).parent
    ...
    # Intel GPU 확인
    logger.info("=" * 80)
    logger.info("Intel GPU LogBERT 학습 스크립트")
    logger.info("=" * 80)
    
    if not IPEX_AVAILABLE:
        logger.warning("⚠️  Intel Extension for PyTorch가 설치되지 않았습니다.")
        logger.warning("설치: pip install intel-extension-for-pytorch")
        logger.warning("CPU 모드로 계속 진행합니다...")
    ...
    if 'output_dir' not in config:
        script_dir = Path(__file__).parent
        config['output_dir'] = str(script_dir.parent / 'checkpoints')
    ...
    dataloader = create_dataloader(
        ...
        pin_memory=False,  # Intel GPU에서는 pin_memory 비활성화
    )
    ...
    # 학습기 생성
    logger.info("\n" + "=" * 80)
    logger.info("Intel GPU 학습기 초기화...")
    logger.info("=" * 80)
    
    trainer = IntelLogBERTTrainer(config)
