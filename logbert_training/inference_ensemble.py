...
class EnsembleInference:
    ...
    def __init__(self, checkpoint_path: str, device: str = None):
        """
        Args:
            checkpoint_path: 앙상블 모델 체크포인트 경로
            device: 사용할 디바이스
        """
        if device is None:
        ...
        logger.info(f"앙상블 모델 로드 완료: {checkpoint_path}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='앙상블 모델 추론')
    parser.add_argument('--checkpoint', type=str, required=True,
                       help='앙상블 체크포인트 파일 경로')
    ...
    inference = EnsembleInference(args.checkpoint)
